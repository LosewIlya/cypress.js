describe('Покупка нового автара', function () {
    ...
    it('Покупка нового автара для своего тренера ', function () {
         ...
         cy.get('section.shop').find('ul li').eq(1).find('button').click();// Нажал кнопку "Купить"
         cy.get('form').should('be.visible');
         cy.get('input[autocomplete="cc-number"], input[name*="card"], [data-test*="card"], [data-testid*="card"], .payment_form_card_form input[type="text"], .style_1_base_input').first().clear().type('4003600000000014', { delay: 20 }); // Найти поле с номером карты и ввести номер карты
         cy.get('input[class="style_1_base_input card_date"]').type('1226'); // Найти поле с датой карты и ввести дату
         cy.get('input[class="style_1_base_input card_csv"]').type('125'); // Найти поле с кодом и ввести код сvv
         cy.get('input[class="style_1_base_input card_name"]').type('german dolnikov'); // Найти поле с именем и ввести имя
         cy.get('.style_1_base_button_payment_body > .style_1_base_button_payment').click(); // Нажал кнопку "оплатить"
         cy.get('input[class="style_1_base_input threeds_number"]', { timeout: 15000 }).should('be.visible')  // Сначала проверяем, что элемент есть в DOM.should('be.visible');  
         cy.get('input[class="style_1_base_input threeds_number"]').type('56456') // Найти поле и ввести код из смс
         cy.get('.style_1_base_button_payment_body button').should('be.visible');
         cy.get('.style_1_base_button_payment_body button').should('be.visible').click();// Нажал кнопку "оплатить"
